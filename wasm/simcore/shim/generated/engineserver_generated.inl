	...
	int IsMapValid( const char *filename ) override { return (int)0; }
	int IsInEditMode( void ) override { return (int)0; }
	int PrecacheSentenceFile( const char *s, bool preload ) override { return (int)0; }
	...
	int GetClusterForOrigin( const Vector &org ) override { return (int)0; }
	int GetPVSForCluster( int cluster, int outputpvslength, unsigned char *outputpvs ) override { return (int)0; }
	...
	int GetPlayerUserId( const edict_t *e ) override { return (int)0; }
	...
	int SentenceGroupPick( int groupIndex, char *name, int nameBufLen ) override { return (int)0; }
	int SentenceGroupPickSequential( int groupIndex, char *name, int nameBufLen, int sentenceIndex, int reset ) override { return (int)0; }
	int SentenceIndexFromName( const char *pSentenceName ) override { return (int)0; }
	const char * SentenceNameFromIndex( int sentenceIndex ) override { return 0; }
	int SentenceGroupIndexFromName( const char *pGroupName ) override { return (int)0; }
	...
	int CompareFileTime( const char *filename1, const char *filename2, int *iCompare ) override { return (int)0; }
	...
	int CheckHeadnodeVisible( int nodenum, const byte *pvs, int vissize ) override { return (int)0; }
	int CheckAreasConnected( int area1, int area2 ) override { return (int)0; }
	int GetArea( const Vector &origin ) override { return (int)0; }
	...
	const CBitVec<MAX_EDICTS>* GetEntityTransmitBitsForClient( int iClientIndex ) override { return 0; }
	...
	int IsInCommentaryMode( void ) override { return (int)0; }
	...
	QueryCvarCookie_t StartQueryCvarValue( edict_t *pPlayerEntity, const char *pName ) override { return (QueryCvarCookie_t)0; }
	...
	int GetClusterCount(  ) override { return (int)0; }
	int GetAllClusterBounds( bbox_t *pBBoxList, int maxBBox ) override { return (int)0; }
	edict_t * CreateFakeClientEx( const char *netname, bool bReportFakeClient ) override { return 0; }
	int GetServerVersion(  ) const override { return (int)0; }
	...
	eFindMapResult FindMap( char *pMapName, int nMapNameMax ) override { return (eFindMapResult)0; }

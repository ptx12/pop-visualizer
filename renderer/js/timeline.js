...
function buildActivity(span, pps, sim, wave) {
  ...
  for (const ws of wave.wavespawns) {
    ...
    for (const ev of r.events) drops.push([ev.t + r.life, perBot * ev.count]);
  ...
  if (drops.length) {
    ...
    cpts.push(`${(Math.min(span, drops[drops.length - 1][0]) * pps).toFixed(1)},${(H - cum / totalCash * (H - 4)).toFixed(1)}`);
    cashPts = cpts.join(' ');
  }
